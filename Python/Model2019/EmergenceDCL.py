...
class Experiment(object):
	...
	def run_dyad(self, TO_FILE=True, DEB=False):
		...
		Players[0].strategy = self.strategies[3]
		# Players[1].strategy = self.strategies[4]
...
###########################################################################################
			...
			for k in range(0, Pl):
				...
				dfAux = dfAux[['Dyad','Round','Player','Answer','Time','a11','a12','a13','a14','a15','a16','a17','a18','a21','a22','a23','a24','a25','a26','a27','a28','a31','a32','a33','a34','a35','a36','a37','a38','a41','a42','a43','a44','a45','a46','a47','a48','a51','a52','a53','a54','a55','a56','a57','a58','a61','a62','a63','a64','a65','a66','a67','a68','a71','a72','a73','a74','a75','a76','a77','a78','a81','a82','a83','a84','a85','a86','a87','a88','Score','Joint','Is_there','where_x','where_y']]
				# print(dfAux)
			...
			if place == -1:
				if DEB:
					print("*********")
					print("Estimating new strategy player 1:")
				try:
					Players[0].strategy = FRA.estimate_strategy(reg1, Players[0].score, j, self.modelParameters[:10], self.regions, self.strategies, DEB)
				except:
				...
				if DEB:
					print("*********")
					print("Estimating new strategy player 2:")
				try:
					Players[1].strategy = FRA.estimate_strategy(reg2, Players[1].score, j, self.modelParameters[10:], self.regions, self.strategies, DEB)
				except:
					j = FRA.code2Vector(both, Num_Loc)
	...
	def run_simulation(self, TO_FILE=True, DEB=False):
		IT = self.gameParameters[4] # number of experiments in a set
		for h in range(0, IT):
			print("****************************")
			print("Running dyad no. ", h + 1)
			print("****************************\n")
			self.run_dyad(TO_FILE=TO_FILE, DEB=DEB)
