...
plot_FocalTransitions2 <- function(df) {
...
plot_FRA_Transitions <- function(df) {
  
#  df <- df[df$Region == 'RS', ]
  df <- df[df$RegionGo == 'Focal', ]
  
  min_FRA <- min(df$MaxFRASim)
  max_FRA <- 1.5
  alpha <- 0.5
  
  g <- ggplot() +
    geom_point(aes(x = MaxFRASim, y = Freqs), df, alpha = alpha, size=1.5) + 
    scale_x_continuous(limits = c(min_FRA, max_FRA)) + 
    scale_y_continuous(limits = c(0, 1.01)) + 
    xlab("Max. FRAsim") +
    #  ylab("") +
    ylab("Rel. Freq./Attractiveness") +
    ggtitle("Attraction to focal") +
    theme_bw()
...
plot_ModelTransitions_FRA <- function(theta, pl, plColor) {
  xs <- seq(0,1.5,length.out=200)
  fitFRA <- sapply(xs, FRAattract, theta=theta)
  dfB <- data.frame(xs, fitFRA)
  pl <- pl + geom_line(aes(x = xs, y = fitFRA), dfB, color=plColor, size=0.7)
  return(pl)
}

plot_ModelTransitions_Focal <- function(theta, pl, plColor) {
  
  xs <- seq(-65,32,length.out=200)
  # print('xs')
  # print(xs)
  # regiones <- c('ALL', 'NOTHING', 
  #               'BOTTOM', 'TOP', 'LEFT', 'RIGHT',
...
plot_ModelTransitions_Focal_FRA <- function(theta, pl, plColor) {
  
  xs <- seq(-65,32,length.out=200)
  fitFocal <- sapply(xs, FRAprob_Focal, frasim=1, theta=theta)
  dfB <- data.frame(xs, fitFocal)
  pl <- pl +
    geom_line(aes(x = xs, y = fitFocal), dfB, color=plColor, size=0.7)
